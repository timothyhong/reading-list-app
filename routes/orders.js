...
router.route("/").get((req, res) => {
	...
	getOrders(req.query.order_id, req.query.customer_id).then(rows => {
    	...
    	context.message = "Displaying " + context.orders.length + " of " + context.orderIds.length + " order(s).";
   		context.message2 = "Displaying " + context.orderProducts.length + " product(s) across " + context.orders.length + " order(s).";
